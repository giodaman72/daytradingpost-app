...
export function evaluateProductionReadiness(env) {
  ...
  const marketDataProvider = env.MARKET_DATA_PROVIDER?.trim().toLowerCase();
  if (
    marketDataProvider !== "disabled" &&
    marketDataProvider !== "generic_http"
  )
    errors.push(
      "Market data: MARKET_DATA_PROVIDER must be disabled or generic_http in production.",
    );
  if (marketDataProvider === "generic_http") {
    requireKeys(
  ...
  if (marketDataProvider === "disabled")
    warnings.push(
      "Market-data quotes are disabled; quote surfaces will show Data unavailable.",
    );
  ...
  if (
    present(env, "PURCHASE_EMAIL_FROM") &&
    !/<[^<>\s@]+@[^<>\s@]+\.[^<>\s@]+>$/.test(env.PURCHASE_EMAIL_FROM.trim())
  )
    errors.push(
      "Payments: PURCHASE_EMAIL_FROM must use the format Name <email@example.com>.",
    );
