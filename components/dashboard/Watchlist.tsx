      ...
      <ul className="dashboard-watchlist">
        {DEFAULT_WATCHLIST.map((item) => (
          <li key={item.symbol}>
            ...
            <span className="watchlist-status"><i aria-hidden="true" />Tracking</span>
          </li>
        ))}
      </ul>
      <p className="dashboard-data-note">Instrument watchlist only · No live pricing connected</p>
      <Link href="/analysis" className="text-link">Open market analysis →</Link>
    </DashboardPanel>
  );
}
