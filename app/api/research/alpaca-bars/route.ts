...
import {
  ...
  getAlpacaDailyBars,
} from "@/lib/backtesting/alpacaHistoricalData";
import { isAlpacaResearchSymbol } from "@/lib/backtesting/alpacaHistoricalMapper";
...
export async function GET(request: Request) {
  ...
  if (!isAlpacaResearchSymbol(symbol))
    return Response.json(
      { code: "INVALID_SYMBOL", message: "Select SPY, QQQ, or GLD." },
  ...
  if (rangeDays < MIN_RANGE_DAYS || rangeDays > MAX_RANGE_DAYS)
    return Response.json(
      {
        code: "INVALID_RANGE",
        message: "Choose a historical range between 120 days and 10 years.",
      },
      { status: 400, headers: { "Cache-Control": "no-store" } },
    );

  try {
    const result = await getAlpacaDailyBars(
      symbol,
      startValue!,
      endValue!,
      request.signal,
    ...
    const normalized =
      error instanceof AlpacaHistoricalDataError
        ? error
        : new AlpacaHistoricalDataError(
            "INTERNAL_ERROR",
